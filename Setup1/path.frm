...
Private Sub cmdOK_Click()
    ...
    If strPathName <> vbnullstring Then
        If (frmSetup1.Tag = gstrDIR_DEST And strPathName <> gstrDestDir) Or _
                frmSetup1.Tag = gstrDIR_DB Or frmSetup1.Tag = gstrDIR_Back Then
...
Private Sub Form_Load()
    ...
    If frmSetup1.Tag = gstrDIR_SRC Then
        ...
        End If
        mfMustExist = True
    ElseIf frmSetup1.Tag = gstrDIR_DB Then
        Caption = "Data base Path"
        lblPrompt.Caption = "Please enter the path to keep the database."
        mfCancelExit = True
        dirDirs.Path = gstrDBDir
        If Err > 0 Then
            dirDirs.Path = Left$(App.Path, 3)
        End If
        ...
        mfMustExist = True
    ElseIf frmSetup1.Tag = gstrDIR_Back Then
        Caption = "Back up path"
        lblPrompt.Caption = "Please enter the path to keep the back up database ."
        mfCancelExit = True
        ...
        txtPath.Text = gstrDestDir & IIf(Right(gstrDestDir, 1) = "/", "", "/") & "BackUp"
        mfMustExist = True
    Else
        Caption = ResolveResString(resCHANGEDIR)
        lblPrompt.Caption = ResolveResString(resDESTPROMPT)
        mfCancelExit = False
        dirDirs.Path = gstrDestDir & "BackUp\"
        If Err > 0 Then
    ...
    If frmSetup1.Tag <> gstrDIR_DEST Then
        txtPath.Text = dirDirs.Path
    End If
