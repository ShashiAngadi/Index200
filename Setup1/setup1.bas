...
'Const Usede By waves INdex 2000
Global Const gstrDIR_DB$ = "DB"
Global Const gstrDIR_Back$ = "B"
''''
...
'''Variable used by index 2000 Appliaction
Global gstrDBDir As String                                'dir for Database  files
Global gstrDBackUpDir As String                           'dest dir for Data back up files
Global gboolServer As Boolean                             'dest dir for Data back up files
Global gDemoVersion As Boolean                             'Idenitifis the whether Demo versin or not
Global gstrServerName As String                           'Name of the remote computer
Global gstrShareName As String                            'Share name of the remote machine where datab ase is copied
Global gstrLangName As String                             'name of the the langauge used in the data base
'
...
End Sub
'PreviosyInstallaton 'Created By Shashi Angadi
'                    'This Funciton Check the Previos installations
'                    while installing it creates Some registry entry
'                    if thereasr is any such entried are there
'                    Then it checks for the Appliaction Path
'                    If appliaction path exists then the appliaction has
'                    already installed (Not Implemented)
'
'       Returns True if appliaction is already installed otherwise false
'
Function PreviousInstallation() As Boolean

Dim strPath As String

Const cRegKey = "Software\Waves Information Systems\Index 2000V3\Settings"

' Create the new key, whose name is based on the app's name
strPath = GetRegistryValue(HKEY_LOCAL_MACHINE, cRegKey, "ProductID")
...
If Len(strPath) Then
    ...
    strPath = GetRegistryValue(HKEY_LOCAL_MACHINE, strPath, "Path")
    'This Appliaction has already installed it need no to install once again
    'So stop the installation 'And Exit the appliaction
    If Len(strPath) Then
        MsgBox "This product has alrady installsed in the path " & strPath, , "Inst Error"
    Else
        MsgBox "The product already has installed", , "Installation Error"
    End If
End If

End Function

'This function Created by shashi Angadi
'this Add the information of the index 2000
'to the registry like Langauge
'Name of the machine where data base kept
'Database path
Sub RegisterIndex2000()
    
''Now get the Product Code generated by system
Const constWis = "software\Waves Information Systems"
Const constwis_Index = "Software\Waves Information Systems\Index 2000V3"
Const constwis_Setting = "Software\Waves Information Systems\Index 2000V3\Settings"
...
ExpiryDate = Format(DateAdd("yyyy", 1, Now), "dd/mm/yyyy")

' Create the new key, whose name is based on the app's name
ProductKey = GetRegistryValue(HKEY_LOCAL_MACHINE, constwis_Setting, "ProductID")
If Len(ProductKey) Then
    'This Appliaction has already installed it need no to install once again
    'So stop the installation 'And Exit the appliaction
    MsgBox "The product already has installed", , "Installation Error"
...
For Count = 1 To 4
    Randomize
    strTemp = Format(Rnd(1) * 10000, "0000")
    ProductKey = ProductKey & "-" & strTemp
Next
ProductKey = Mid(ProductKey, 2)
                
strwisRegKey = "Software\Waves Information Systems\" & ProductKey
'First create the Product Registry key

'Create Key in the HKey_LOACAL_MACHINE registry
If Not RegCreateKey(HKEY_LOCAL_MACHINE, constWis, "Index 2000V3", hKey) Then GoTo Err_line
If Not RegCreateKey(HKEY_LOCAL_MACHINE, constwis_Index, "Settings", hKey) Then GoTo Err_line
hLm = hKey
'Create Key in the HKEY_CURRENT_USER Registry
If Not RegCreateKey(HKEY_CURRENT_USER, constWis, ProductKey, hKey) Then GoTo Err_line
hCU = hKey


'Now Register the information of language & Product
If Not RegSetStringValue(hLm, "Language", gstrLangName) Then GoTo Err_line
If Not RegSetStringValue(hLm, "ProductID", ProductKey) Then GoTo Err_line
If Not RegSetStringValue(hLm, "InstallDate", InstallDate) Then GoTo Err_line

'Now Get the Product Value
MaxCount = Len(ProductKey)
For Count = 1 To MaxCount
    strTemp = Mid(ProductKey, Count, 1)
...
'Create Key in the HKEY_CURRENT_USER Registry
If Not RegSetStringValue(hCU, "", ProductVal) Then GoTo Err_line
If Not RegSetStringValue(hCU, "License", ProductVal) Then GoTo Err_line


'If its not Demo version then no of days to the Trial is 0
Count = IIf(gDemoVersion, 30, 0)

If Not RegSetStringValue(hCU, "Validity", Count) Then GoTo Err_line
If Not RegSetStringValue(hCU, "Days", Count) Then GoTo Err_line
If Not RegSetStringValue(hCU, "Count", "0") Then GoTo Err_line
If Not RegSetStringValue(hCU, "ExpiryDate", ExpiryDate) Then GoTo Err_line

''Now Register the information of the Data base & it's path
If gboolServer Then
    If Not RegSetStringValue(hLm, "DBFolderName", gstrDBDir) Then GoTo Err_line
    If Not RegSetStringValue(hLm, "BackUpFolder", gstrDBackUpDir) Then GoTo Err_line
Else
    If Not RegSetStringValue(hLm, "ServerName", gstrServerName) Then GoTo Err_line
    If Not RegSetStringValue(hLm, "ShareName", gstrShareName) Then GoTo Err_line
End If
...
'This function GGet the information about
'whether the software is installing at the client
'machine or server machine in case of
'if it is client machine then need not to install the database
'
'
Function ShowServerComponent() As Boolean

    frmComponent.Show vbModal

'If it is the client machine then ask for the
'name of the server ans share name of the database path
If Not gboolServer Then
    frmServer.Show vbModal
End If

ShowServerComponent = gboolServer
...
'
'-----------------------------------------------------------
Function RegCreateKey(ByVal hKey As Long, ByVal lpszSubKeyPermanent As String, _
                    ByVal lpszSubKeyRemovable As String, phkResult As Long) As Boolean
    Dim lResult As Long
...
' Returns: True on success, else False.
'
Function RegSetNumericValue(ByVal hKey As Long, ByVal strValueName As String, _
            ByVal lData As Long, Optional ByVal fLog As Boolean = True) As Boolean
    Dim lResult As Long
    Dim strHKey As String
    ...
    'RegSetValueEx(hKey, szSubKey, 0, REG_SZ, ByVal szValueKey, CLng(Len(szValueKey)))
    If lResult = ERROR_SUCCESS Then
...
Function RegSetStringValue(ByVal hKey As Long, ByVal strValueName As String, _
                ByVal strData As String, Optional ByVal fLog As Boolean = True) As Boolean
    Dim lResult As Long
    Dim strHKey As String
...
Sub ShowPathDialog(ByVal strPathRequest As String)
    ...
    ElseIf strPathRequest = gstrDIR_DB Then
        gstrDBDir = frmSetup1.Tag
    ElseIf strPathRequest = gstrDIR_Back Then
        gstrDBackUpDir = frmSetup1.Tag
    Else
        If gfRetVal = gintRET_CONT Then
